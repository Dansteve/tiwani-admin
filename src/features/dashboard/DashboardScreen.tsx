...
// aggregate signup-trend chart, and a recent-activity table (the always-visible data-source banner is in
// the (admin) layout, so it is not repeated here). It reads through the admin-api client
// (src/lib/admin-api/client.ts) via TanStack Query, exactly the pattern the real screens will use, so the
// seam is exercised now; the client currently delegates to the MOCK adapters (Decisions.md D16). When the
// audited admin-api lands, only the client body changes, not this screen. Everything shown is
// aggregate-only (counts, no identified individuals; README red line 9).
